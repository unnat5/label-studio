...
ARG BRANCH_OVERRIDE
ARG SKIP_VERSION_GEN=true

################################ Overview
# Label Studio Dockerfile with updated build pipeline
################################
...
RUN if [ "$SKIP_VERSION_GEN" = "true" ]; then \
      ...
      echo "Frontend version generation not available in this build"; \
    fi
...
# SKIP py-version-generator due to missing .git
...
RUN mkdir -p $LS_DIR $LABEL_STUDIO_BASE_DATA_DIR $OPT_DIR && \
